...
public class ReservationControllerTest {
    ...
    @Test
    @DisplayName("GET /reservation/1 - Found")
    void shouldGetReservationById() throws Exception {
        ...
        mockMvc.perform(get("/reservation/{id}", 1))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(header().string(HttpHeaders.LOCATION, "/reservation/1"))
    ...
    @Test
    @DisplayName("GET /reservation/1 - Not Found")
    ...
    void shouldDeleteReservation() throws Exception {
        doReturn(Optional.of(mockReservation)).when(reservationService).findById(1L);
        doReturn(true).when(reservationService).delete(1L);
