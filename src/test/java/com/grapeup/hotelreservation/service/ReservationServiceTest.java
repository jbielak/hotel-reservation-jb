...
public class ReservationServiceTest {
    ...
    public void shouldUpdateReservationThenReturnReservationWithReassignedRoomId() {
        ...
        Room roomToUpdate = Room.builder().id(1L).roomType(RoomType.BASIC)
                .reservations(new HashSet<>(Arrays.asList(reservationToUpdate))).build();
        reservationToUpdate.setRoom(roomToUpdate);
        Room newRoom = Room.builder().id(2L).roomType(RoomType.SUITE)
                .reservations(new HashSet<>(Arrays.asList(reservationToUpdate))).build();
    ...
    public void shouldReturnEListOfReservationsForRoom() {
        Reservation reservation2 = Reservation.builder().id(2L).username("test")
                .numberOfPeople(3).startDate(LocalDate.of(2020, 8, 1))
                .endDate(LocalDate.of(2021, 9, 1)).build();
        Room room = Room.builder().id(1L).roomType(RoomType.BASIC)
                .reservations(new HashSet<>(Arrays.asList(reservation2))).build();
